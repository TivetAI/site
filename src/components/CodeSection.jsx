...
import { Icon, faCheck } from "@tivet-gg/icons";
...
export default function CodeSection() {
	...
	const changePage = (i) =>
		setPage({ index: i, dir: i > page.index ? 1 : -1 });
		...
		<div className="flex flex-col items-center gap-8 px-4">
			...
			<ul className="mx-auto list-none text-cream-100">
				{[
					...
					<>
						Deploy existing multiplayer games using integrations
						with{" "}
				...
				<motion.div
					...
					onDragEnd={(e, { offset, velocity }) => {
						...
						if (swipe < -swipeConfidenceThreshold) {
							...
							onChangePage(
								paginate(page.index, -1, ENGINE_PAGES),
							);
						}
					}}
				>
					<EnginePageContents
						page={ENGINE_PAGES[page.index]}
						scale={page.index === 3}
					/>
				</motion.div>
			</AnimatePresence>
...
function paginate(page, dir, arr) {
	...
	if (newPage < 0)
		return { index: arr.length - (-newPage % arr.length), dir };
