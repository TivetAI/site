...
type LiteralSchema = { type: "literal"; value: string };
...
function PropertyTypeLabel(props: PropertyTypeLabelProps) {
	return (
		<div className="text-xs opacity-20">{getPropertyTypeLabel(props)}</div>
	);
}
	...
	return (
		...
		>
			{Object.keys(schema.properties).map((key) => {
				...
				property =
					property.type === "nullable" ? property.item : property;
				property =
					property.type === "optional" ? property.value : property;
				...
				if (
					...
					const isEmpty =
						property.elementType.type === "object" &&
						Object.keys(property.elementType.properties).length ===
							0;
					return (
						<ObjectSchemaItem key={key}>
							<PropertyLabel
				...
				if (property.type === "array") {
					...
					const isEmpty =
						...
						Object.keys(property.item.properties).length === 0;
