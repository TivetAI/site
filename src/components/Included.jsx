...
export function IncludedFeature({ icon, title, usually, on }) {
	return (
		<div className="flex flex-col items-start border border-cream-100/5 px-10 py-12 text-left">
			...
			{/* Content */}
			<div>
				<div className="text-lg font-semibold leading-tight text-cream-100">
					{title}
				</div>
				{usually && (
...
const OTHER_SERVICES_PRICE =
	(10 + 0.5) * OTHER_SERVICES_SERVER_COUNT +
	9 * OTHER_SERVICES_USER_COUNT +
	26 +
	40;
	...
	return (
		<div className="mx-auto flex max-w-6xl flex-col px-4">
			...
			>
				...
				<IncludedFeature
					icon={faLock}
					title="Automatic SSL for WebSockets & TLS"
				/>
				<IncludedFeature
				...
				<div className="text-md col-span-full flex flex-col items-center whitespace-pre border border-cream-100/5 px-10 py-12 font-mono text-cream-100 md:text-xl">
					<div>
						...
						{" ".repeat(
							receiptWidth -
								otherServicesText.length -
								otherServicesPrice.length,
						)}
						<Tooltip tip="Assuming 15 servers & 4 users">
							{otherServicesPrice}
						</Tooltip>
					</div>
					<div>
						<span className="font-bold">Tivet</span> $0.00
					</div>
				</div>
			</div>
		</div>
