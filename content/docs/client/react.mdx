...
First, add Tivet's Actor Client SDK to your project.

<CodeGroup>

```bash {{ "title": "npm" }}
npm i @tivet-gg/actor-client --save
```
...
```bash {{ "title": "jsr" }}
npx jsr add @tivet-gg/actor-client
```
</CodeGroup>

[Generate a Actor Manager endpoint by using the `tivet` CLI and guide here](https://tivet.gg/docs/setup). Then add the generated URL to your env file.
...
import { ActorClientProvider } from "@tivet-gg/actor-client/unstable-react";
...
`useActor` is a hook that allows you to interact with actors in your React components. Pass to it a name of your actor, [and optionally any other create parameters](https://tivet.gg/docs/manage#options). It will connect to the actor, once the component is mounted, and disconnect when the component is unmounted. 
...
Optionally, if you're using TypeScript, you can pass your Actor class type to the hook to get the correct types for [RPC methods](https://tivet.gg/docs/rpc). 

<CodeGroup>
```tsx {{"title": "JavaScript"}}
import { useActor } from "@tivet-gg/actor-client/unstable-react";
...
In order to render components from your Actor, you need to ensure that your Actor extends the `RscActor` class from the `@tivet-gg/actor` package. Your RPC methods can receive props like any other React component.

```tsx {{"title": "your-actor.tsx"}}
import { Rpc } from "@tivet-gg/actor";
import { RscActor } from "@tivet-gg/actor/unstable-react";  // [!code focus]
...
```tsx {{ "title": "your-actor.tsx" }}
import { Rpc } from "@tivet-gg/actor";
import { RscActor } from "@tivet-gg/actor/unstable-react";
...
import { 
  unstable_createActorHooks
} from "@tivet-gg/actor-client/unstable-react";
