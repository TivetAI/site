...
export * from "@tivet-gg/components/mdx";
export const a = Link;
...
export function AutomateWithApi() {
	return (
		<Accordion title="Can I automate this with APIs?">
			<p>
				Tivet&apos;s Cloud API can be managed with your{" "}
				<Link href="/docs/general/concepts/token-types#cloud">
					cloud token
				</Link>
				. This is the same API we use internally and{" "}
				<Link href="https://github.com/tivet-gg/cli/blob/7f91d180f64b755956d89a3a83bfb2e77a42d72d/cli/src/commands/version.rs#L368">
					in the CLI
				</Link>
				.
			</p>
			...
			<p>
				...
				<Link
					href="https://www.npmjs.com/package/@tivet-gg/api-full"
					target="_blank"
				>
					<code>@tivet-gg/api-full</code> NPM library
				</Link>{" "}
...
export function WhatDoesTivetDo() {
	return (
		<Accordion title="What did this do?">
			<ul>
				<li>Linked your project to Tivet</li>
				<li>
					If no <code>tivet.yaml</code> was present, it created a{" "}
					<code>tivet.yaml</code> file that configures how to run your
					game. Read more about the Tivet version config{" "}
					<a href="/docs/general/concepts/version-config">here</a>.
				</li>
				<li>
					Added a development token to your <code>.env</code> that
					lets you develop with Tivet on your local machine. Read more
					about dev tokens{" "}
					<a href="/docs/general/concepts/dev-tokens">here</a>.
				</li>
			</ul>
			<p>
				You can also run <code>tivet init</code> without any flags to go
...
export const Version = () => {
	...
	return (
		<CodeBlock
			lang="bash"
			code={`deno install -n opengb -fgA https://raw.githubusercontent.com/tivet-gg/opengb/v${version}/src/cli/main.ts`}
		/>
	);
};
...
export { Step, Steps } from "@tivet-gg/components";
